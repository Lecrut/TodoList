...
import { collection, where, query, doc, setDoc, deleteDoc , onSnapshot, getDocs, updateDoc } from 'firebase/firestore'
...
export const useTasksStore = defineStore('counter', () => {
  ...
  onSnapshot(TasksCollectionRef, docsSnap => {
    ...
    docsSnap.forEach(doc => {
      todos.value.push({ id: doc.data().id, text: doc.data().text, done: doc.data().done, edit: doc.data().edit, docId: doc.data().docId });
    })
  ...
  async function addTodo() {
    if (newTodo.value.length > 3) {
      await setDoc(doc(collection(db, "tasks")), { id: id++, text: newTodo.value.toUpperCase(), done: false, edit: false, IdUsera: userID, docId: '' })
      id--
  ...
  return {showTodo, sortList, makeMade, changeTodo, removeTodo, editTodo, addTodo, filteredTodos, todos, hideCompleted, newTodo, newName, id, nameRules}
})
